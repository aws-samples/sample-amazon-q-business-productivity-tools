...
import { Sparkles, Send, MessageSquare, Loader } from 'lucide-react';
...
const AIResponsePanel: React.FC<AIResponsePanelProps> = ({
  ...
  const [isProcessingStream, setIsProcessingStream] = useState<boolean>(false);
  ...
  const handleChatContinue = async () => {
    ...
    // Add empty assistant message immediately to show loading indicator
      ...
      >
        {isAiResponseLoading && !streamingMessage ? (
          ...
          <div className="space-y-6">
            ...
            {isChatting && !streamingMessage && !chatMessages.some((msg) => msg.isStreaming) && (
